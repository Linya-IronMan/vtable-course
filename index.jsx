...
const toggleCheckboxVisible = (target, checkboxVisible, textVisible) => {
	if (
		target.forEachChildren &&
		typeof target.forEachChildren === "function"
	) {
...
const calcCellTags = (tags = [], options, tableInstance) => {
	...
	if (len === 1) {
		...
		for (let i = 0; i < len; i += 1) {
			const { item, tagMaxWidth, tagMinWidth, textLength, tagWidth } =
				formatTagItem(data[i], options, tableInstance);

			const handlePushItem = (item) => {
				const calcWidth =
					totalWidth +
					tagMinWidth +
					(i === len - 1 ? 0 : numberTagWidth);
				if (tagMaxWidth >= calcWidth) {
					const realTextWidth = Math.min(
						tagWidth,
						tagMaxWidth -
							totalWidth -
							(i === len - 1 ? 0 : numberTagWidth)
					...
					if (
						tagMaxWidth <
						totalWidth + tagMinWidth + numberTagWidth
					) {
						addTags({
					...
					if (i === len - 1) {
						item.tagProps.width = Math.min(
							tagWidth,
							tagMaxWidth - totalWidth
						);
						addTags(item);
					} else {
						item.tagProps.width = Math.min(
							tagWidth,
							tagMaxWidth - totalWidth - numberTagWidth
						);
						addTags(item);
						addTags({
							label: "+" + (len - filterTags.length),
...
const calcRowCount = (cellHeight, options) => {
	const {
		lineHeight = 24,
		paddingTop = 4,
		paddingBottom = 4,
	...
	const rows =
		Math.floor((availableHeight - lineHeight) / (lineHeight + lineGap)) + 1;
			...
			customLayout: (args) => {
				...
				const container = (
					...
					>
						...
						>
							<VText
								attribute={{ text: record.index, fill: "#000" }}
							/>
						</VGroup>
					</VGroup>
				);
